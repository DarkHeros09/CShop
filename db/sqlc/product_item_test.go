...
func TestListProductItemsV2OrderByHighPrice(t *testing.T) {
	...
	require.NotEmpty(t, secondPage)
	require.Equal(t, 10, len(secondPage))
	p1, err := strconv.ParseFloat(initialSearchResult[len(initialSearchResult)-1].Price, 64)
	require.NoError(t, err)
	p2, err := strconv.ParseFloat(secondPage[len(secondPage)-1].Price, 64)
	require.NoError(t, err)
	...
	p3, err := strconv.ParseFloat(secondPage[len(secondPage)-1].Price, 64)
	require.NoError(t, err)
	p4, err := strconv.ParseFloat(thirdPage[len(thirdPage)-1].Price, 64)
	require.NoError(t, err)
	require.GreaterOrEqual(t, p3, p4)
	// require.Greater(t, initialSearchResult[len(initialSearchResult)-1].ID, thirdPage[len(thirdPage)-1].ID)
